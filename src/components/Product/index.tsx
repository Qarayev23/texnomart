...
const Product = ({ product, sort }: ProductCardProps) => {
    ...
    const productURLFriendly = stringToURLFriendly(product.name)
    ...
    return (
        <div className={sort?.row ? `${styles.product} ${styles.row}` : styles.product} >
            ...
            <Link to={`/${product.category}/${productURLFriendly}`} state={product.name} className={styles.product__img}>
                <img src={product.img[0]} alt={product.name} />
            </Link>
            <div className='w-full'>
                <Link to={`/${product.category}/${productURLFriendly}`} state={product.name} className={styles.product__name}>
                    <h4>{product.name}</h4>
                </Link>
